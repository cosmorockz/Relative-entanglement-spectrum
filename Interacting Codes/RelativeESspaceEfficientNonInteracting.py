...
InitCols = [FockBasisToDecimal(wf)]
InitData = [1]

URot1 = FullRotation(Mz)

def StateRotation1(state):
    state = DecimalToFockBasis(state)
    WfSp = PSWfSegmentor(state)
    fullWf = []
    for k,v in WfSp.items():
        if fullWf == []:
            fullWf = np.linalg.inv(URot1[k])@WfSp[k]
        else:
            fullWf = sparse.kron(fullWf,np.linalg.inv(URot1[k])@WfSp[k])
#     WfRo = WfRo + coef * fullWf
    return fullWf

WfRo1 = sparse.csr_matrix(([0],([0],[0])),shape=(1,2**NumberofStates))
with concurrent.futures.ProcessPoolExecutor() as executor:
    results = executor.map(StateRotation1,InitCols)
    
    for i,result in enumerate(results):
        WfRo1 += InitData[i] * result
        
logging.debug("Rotation 1 done \n")

print(process.memory_info().rss/(1024**3))
print("Rotation 1 done")

WfRo1 = WfRo1.tocoo()
WfRoRows1 = WfRo1.row
WfRoCols1 = WfRo1.col
WfRoData1 = WfRo1.data
del WfRo1
del URot1
gc.collect()

URot2 = FullRotation(mz)

def StateRotation2(state):
    state = DecimalToFockBasis(state)
    WfSp = PSWfSegmentor(state)
    fullWf = []
    for k,v in WfSp.items():
        if fullWf == []:
            fullWf = URot2[k]@WfSp[k]
        else:
            fullWf = sparse.kron(fullWf,URot2[k]@WfSp[k])
#     WfRo = WfRo + coef * fullWf
    return fullWf

WfRo = sparse.csr_matrix(([0],([0],[0])),shape=(1,2**NumberofStates))
with concurrent.futures.ProcessPoolExecutor() as executor:
    results = executor.map(StateRotation2,WfRoCols1)
    
    for i,result in enumerate(results):
        WfRo += WfRoData1[i] * result

logging.debug("Rotation 2 done \n")

print(process.memory_info().rss/(1024**3))
print("Rotation 2 done")
...
logging.debug("Particle-hole transform done \n")

print(process.memory_info().rss/(1024**3))
print("Particle-hole transform done")
