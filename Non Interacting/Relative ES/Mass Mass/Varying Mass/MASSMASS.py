...
n_cutoff = 10
mz = 10
with open('Upper Half Mass.dat','r') as MyFile:
    Mz = float(MyFile.read())
delPair = 0
delta = 1e+10
...
np.savetxt("LowerHalf Mass"+str(mz)+"/M1_"+str(mz)+"M2_"+str(Mz)+"Cutoff"+str(n_cutoff)+".dat",np.c_[M,EE])
...
plt.figure(1)
plt.scatter(M,EE,s=6,color="#4F628E")
plt.grid(color="#2E4272")
plt.xlabel("m",color="#061539")
plt.ylabel("$\lambda$",color="#061539")
plt.title("$M_{Lower \; Half}=$"+str(mz)+";$M_{Upper \; Half}=$"+str(Mz),color="#061539")
# plt.savefig("LowerHalf Mass"+str(mz)+"/M1_"+str(mz)+"M2_"+str(Mz)+"Cutoff"+str(n_cutoff)+".jpg")
