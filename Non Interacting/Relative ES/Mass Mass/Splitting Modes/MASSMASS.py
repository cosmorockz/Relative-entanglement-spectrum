...
mz = 4
Mz = -4
with open('Splittng Modes Pairing.dat','r') as MyFile:
    delPair = float(MyFile.read())
delta = 1e+10

reader1 = csv.reader(open('SingleBandCutoff_' +str(n_cutoff) +'_M' +str(mz) +'N.csv', "r"))
reader2 = csv.reader(open('SingleBandCutoff_' +str(n_cutoff) +'_M' +str(mz) +'P.csv', "r"))
O1 = {}
O2 = {}
for k,v in reader1:
    O1[k] = v
...
np.savetxt("LowerHalf Mass"+str(mz)+"/M1_"+str(mz)+"M2_"+str(Mz)+"Cutoff"+str(n_cutoff)+"DelPair"+str(delPair)+".dat",np.c_[M,EE])

plt.figure(1)
plt.scatter(M,EE,s=6)
plt.grid()
plt.xlabel("m")
plt.ylabel("$\lambda$")
plt.title("$M_{Lower \; Half}=$"+str(mz)+",$M_{Upper \; Half}=$"+str(Mz))
# plt.savefig("LowerHalf Mass"+str(mz)+"/M1_"+str(mz)+"M2_"+str(Mz)+"DelPair"+str(delPair)+"Cutoff"+str(n_cutoff)+".jpg")
plt.show()
